...
f_model = FastText(min_count=args.mincount,
                   window=args.window,
                   size=args.size,
                   sample=6e-5,
                   alpha=args.alpha,
...
t = time()
f_model.build_vocab(sents_b, progress_per=10000)
logging.info('Time to build vocab: {} mins'.format(to_min(t)))

t = time()
f_model.train(sents_b, total_examples=f_model.corpus_count,
...
# stream the model
f_model.init_sims(replace=True)
